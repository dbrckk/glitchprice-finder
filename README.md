...
npm run check:merge
npm run check:github-mergeable
...
`npm run check:github-mergeable` vérifie que `HEAD` contient déjà `origin/main` ou qu'un merge synthétique avec la branche cible GitHub ne produit aucun conflit de fichier. Le workflow qualité utilise `fetch-depth: 0` pour rendre ce contrôle fiable dans GitHub Actions.

`npm run lock:sync` réaligne uniquement les métadonnées racine de `package-lock.json` avec `package.json` avant vérification pour éviter les conflits GitHub inutiles sur le lockfile. `npm run check:prod-files` refuse les binaires, dossiers de build, dépendances et fichiers locaux non-production suivis par Git. `npm run check:code-hygiene` refuse les placeholders legacy, fins de ligne CRLF, espaces invisibles, caractères de contrôle et espaces finaux. Les scripts `build`, `typecheck`, `test` et `verify` relancent automatiquement `npm ci` si les dépendances locales nécessaires manquent, ce qui évite les erreurs de type `Cannot find module 'vitest'` sur un environnement fraîchement cloné.
...
Avant de mettre à jour la branche sur GitHub, lance `npm run check:merge`, `npm run check:github-mergeable` puis `npm run verify`. Si GitHub affiche encore des conflits sur la PR, utilise `npm run resolve:pr-conflicts -- --base=main --branch=<branche-pr> --push` depuis une copie avec le remote `origin`, ou lance le workflow manuel `Resolve PR Conflicts`. Le dépôt inclut aussi `.gitattributes` pour garder les artefacts générés (`public/live-deals.*`, `package-lock.json`) reviewables en texte sans diff binaire, mais sans merge automatique dangereux sur les sources ou JSON. `docs/merge-readiness.md` résume la procédure de résolution si GitHub signale encore un conflit de branche.
