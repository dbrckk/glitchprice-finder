...
function isConflictFileLine(line) {
  if (!line || /^[0-9a-f]{40,64}$/i.test(line)) {
    return false;
  }

  if (/^(Auto-merging|CONFLICT|Failed to merge|Automatic merge failed)/.test(line)) {
    return false;
  }
...
${result.stderr || ""}`.trim();
  const files = output ? output.split("\n").map((line) => line.trim()).filter(isConflictFileLine) : [];
  return files.length ? [...new Set(files)] : ["unknown conflict"];
}
